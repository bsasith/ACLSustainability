...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errorMsg === '') {
        try {
            ...
            $sql = "INSERT INTO solar_generation_acl_cables
                    (report_month, report_year, solar_kwh, created_by, company_name, energy_type, activity_type)
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            // month(s), year(i), kwh(d), created_by(s), company(s), energy_type(s), activity_type(s)
            $stmt->bind_param("sidssss", $month, $year, $kwh, $username, $company, $energy_type, $activity_type);
            $stmt->execute();
            $stmt->close();
            ...
            header("Location: solar_generation_acl_cables.php?success=1&year={$qYear}&month={$qMonth}&kwh={$qKwh}");
            exit;
...
<head>
    ...
    <title>Solar Electricity Generation (kWh) – ACL Cables PLC</title>
