...
// --------------------
$sql = "SELECT report_month, report_year, solar_kwh
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errorMsg === '') {
        try {
            $sql = "UPDATE solar_generation_acl_cables
                    SET report_month = ?, 
                        report_year = ?, 
                        solar_kwh = ?, 
                        updated_at = NOW()
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sidi", $month, $year, $kwh, $id);
            $stmt->execute();
            $stmt->close();

            header("Location: solar_generation_acl_cables_edit_form.php?id={$id}&success=1");
            exit;
...
<title>Edit Solar Electricity Generation – ACL Cables PLC</title>
...
<div class="card form-card">
    <div class="form-header">
        <h2 class="form-title">
            ...
            Edit Solar Electricity Generation – ACL Cables PLC
        </h2>
        <div class="form-sub">
            ACL Cables PLC | Renewable Energy Generation
        </div>
    </div>
    ...
    <div class="card-body">
        <form method="post">
            ...
            <!-- kWh -->
            <div class="mb-4">
                <label class="form-label fw-bold">Solar Electricity Generation (kWh)</label>
                <input type="number"
                       name="solar_kwh"
                       class="form-control"
                       step="0.01"
                       min="0"
                       value="<?php echo htmlspecialchars($row['solar_kwh']); ?>"
                       required>
            </div>

            <div class="d-flex justify-content-end gap-2">
                <a href="solar_generation_acl_cables_view_edit.php" class="btn btn-outline-secondary btn-submit">
